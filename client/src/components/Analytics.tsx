...
import { 
  Download, Fuel, Calendar, CreditCard, TrendingUp, TrendingDown,
  DollarSign, CheckCircle, CheckCircle2, AlertTriangle, FileText, Lock
} from 'lucide-react';
import { getDashboard, getReports, DashboardData, ReportsData } from '../api';
...
export default function Analytics({ readOnly = false }: { readOnly?: boolean }) {
  ...
  const totalOpCost = reports 
    ? reports.totalFuelCost + reports.totalMaintenanceCost + reports.totalExpenses 
    : 0;

  const fleetUtilization = dashboard?.fleetUtilization || 0;
  
  // SVG dimensions for the circular progress donut
  const radius = 30;
  const strokeWidth = 6;
  const circumference = 2 * Math.PI * radius;
  const strokeDashoffset = circumference - (fleetUtilization / 100) * circumference;
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-md">
        ...
        <div className="bg-white p-lg rounded-2xl border border-[#ebeef8] flex flex-col justify-between hover:border-[#1d4fd5]/30 hover:shadow-md transition-all">
          ...
          <div className="mt-6">
            ...
            <div className="flex items-baseline gap-1 mt-2">
              <h3 className="text-[32px] font-bold text-black tracking-tight">{isLoading ? '--' : Math.round(fleetUtilization)}%</h3>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-lg rounded-2xl border border-[#ebeef8] flex flex-col justify-between hover:border-[#1d4fd5]/30 hover:shadow-md transition-all">
          ...
          <div className="mt-6">
            ...
            <div className="flex items-baseline gap-1 mt-2">
              ...
              <h3 className="text-[32px] font-bold text-black tracking-tight font-mono-md ml-1 inline-block">
                {isLoading ? '--' : totalOpCost.toLocaleString()}
              </h3>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-md">
        ...
        <div className="bg-white p-lg rounded-2xl border border-[#ebeef8] lg:col-span-2 flex flex-col justify-between">
          ...
          <div className="flex-1 min-h-[260px] relative">
            <svg className="w-full h-full min-h-[260px]" viewBox="0 0 600 240" preserveAspectRatio="none">
              ...
              <path d="M 40 180 C 80 170, 80 150, 120 150 C 160 150, 160 170, 200 165 C 240 160, 240 110, 280 100 C 320 90, 320 70, 360 75 C 400 80, 400 130, 440 120 C 480 110, 480 60, 520 50 L 520 200 L 40 200 Z" fill="url(#grossProfitGrad)" />
              <path d="M 40 190 C 80 180, 80 160, 120 155 C 160 150, 160 145, 200 140 C 240 135, 240 125, 280 115 C 320 105, 320 95, 360 92 C 400 90, 400 105, 440 100 C 480 95, 480 85, 520 80" fill="none" stroke="#cbd5e1" strokeWidth="2.5" strokeDasharray="6 4" />
              <path d="M 40 180 C 80 170, 80 150, 120 150 C 160 150, 160 170, 200 165 C 240 160, 240 110, 280 100 C 320 90, 320 70, 360 75 C 400 80, 400 130, 440 120 C 480 110, 480 60, 520 50" fill="none" stroke="#1d4fd5" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" />
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6 pr-12">
              <div className="relative w-20 h-20 flex items-center justify-center shrink-0">
                <svg className="w-20 h-20 -rotate-90">
                  <circle cx="40" cy="40" r={radius} fill="transparent" stroke="#f1f3fe" strokeWidth={strokeWidth} />
                  <circle
                    cx="40" cy="40" r={radius} fill="transparent" stroke="#1d4fd5" strokeWidth={strokeWidth}
                ...
                <span className="absolute text-[15px] font-bold text-black">{Math.round(fleetUtilization)}%</span>
              </div>
